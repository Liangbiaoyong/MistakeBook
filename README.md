...
按热键框选，视觉模型读完直接把公式转成 LaTeX；识别过程中有明确的进度反馈，绝不干等。

![识别中](docs/images/composer-loading.png)

识别完成后进入可编辑的核对界面，模型自评置信度低于 75% 时会强制提醒你复核：

![识别完成](docs/images/composer-filled.png)
...
先只给题干，答案藏在按钮后面 —— 否则没有自测的意义。四档评分回写复习进度。

![复习](docs/images/review.png)

### 统计

分布与趋势，外加由大模型读你自己的错题归纳出的「错因模式」。
